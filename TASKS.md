...
## [x] T06 — 翻訳バッチ: 卷002 周紀二(40年) [Codex]
結果: 卷002 全40年(顯王元年〜四十八年・368→321 BCE)を a〜i の9サブバッチで全 pass 確定。商鞅変法・蘇秦合縱・張儀連衡・孟嘗君の食客・臣光曰(信義論/養賢論/用諫論)を収録。`year_western.py`(ganzhi=0/range=0/accession=0)/`build_view.py`(107 pass)反映済み。
...
### [x] T06i — 卷002 y40(四十八年, 1年/2,060字) [Codex]
結果: 顯王四十八年(庚子・321 BCE)単独年・全 pass。顯王崩御→愼靚王立、燕易王→噲、田嬰=靖郭君封建(海大魚の諫め)、孟嘗君(田文)の由来と食客数千、臣光曰2件(養賢論=孟嘗君批判 /「采葑采菲、無以下體」=用諫の是)、象牀辞退(公孫戌の諫言)、韓宣惠王の二相併用への繆留の諫め(晉六卿・齊簡公・魏犀首/張儀の三例)を収録。Codex/low R1 で **0 件 pass**(クリーン)。受=紂王等の固有名の同定追加は根拠集合外として避け、文=孟嘗君自称は本文内同定として明示。Codex 1回。`year_western.py`(107 records・ganzhi=0/range=0/accession=0、四十七→四十八は連続で seq 不変)/`build_view.py`(107 pass・112 files)反映。
- Done: `data/kb/卷002/j002_y40.json`(pass)。

## [x] T07 — 翻訳バッチ: 卷003 周紀三(23年) [Codex]
結果: 卷003 完走(y01〜y23 全23年 pass)。最終2サブバッチ(T07h/T07i)はドレイン wave で並列確定し統合。`year_western.py` で 314→298 BCE を確定・`build_view.py` 反映。
...
### [x] T07g — 卷003 y14–y15(八年〜九年, 2年/2,457字) [Codex]
結果: 赧王上 八〜九年 全2年 pass。y14(前307・甘茂が宜陽を抜く/秦武王が挙鼎で絶脈薨・昭襄王即位/趙武靈王の胡服騎射・公子成説得)は R1『有之→忘れてはおらぬ』心理追加・『略』完了形誤訳を是正、R2『八子=女官』を妃嬪へ是正、R3 pass(iter3)。y15(前306・武遂返還/甘茂が齊へ出奔/趙の榆中略取・五国へ使者/楚齊韓合従)は R1 で『趙王(武靈王)』同定を巻内連続(y14 確定訳)誤検出と判断し continuity_text 供給で R2 pass(iter2)。continuity y13→y15 連鎖。
- Done: `data/kb/卷003/j003_y14.json`〜`j003_y15.json`(各 pass)。

### [x] T07h — 卷003 y16–y21(十年〜十五年, 6年/2,472字) [Codex]
結果: ドレイン3巡目 wave1 並列(Agent A=卷003 専任)。赧王上 十〜十五年 全6年 pass(305→300 BCE)。y16『力能立昭王』の『力』を手段限定(武力)した誤りを『擁立する力量』へ R2 是正、y21『樗里疾卒、以樓緩爲丞相』に因果『ので』を付した誤りを順接記述へ R2 是正、y17–y20 は R1 一発 pass。continuity y15→y21 連鎖。
- Done: `data/kb/卷003/j003_y16.json`〜`j003_y21.json`(各 pass)。

### [x] T07i — 卷003 y22–y23(十六年〜十七年, 2年/3,190字) [Codex]
結果: ドレイン3巡目 wave1 並列(同 Agent A 連続)。赧王上 十六〜十七年 全2年 pass(299→298 BCE)。y22『王廟見禮畢、出臨朝』の主体を武靈王と誤同定→即位した何が主体へ R2 是正、y23『野雞』を辺りの鶏と種改変→『野の雉』・『公孫龍由是遂詘』に出来事追加→『屈した』へ R2 是正。continuity y21→y23 連鎖。これで卷003 完走。
- Done: `data/kb/卷003/j003_y22.json`〜`j003_y23.json`(各 pass)。

## [x] T08 — 翻訳バッチ: 卷004 周紀四(25年) [Codex]
結果: 卷004 完走(y01〜y25 全25年 pass)。最終サブバッチ T08g(y25)はドレイン wave で並列確定し統合。`year_western.py` で 314→273 BCE を確定・`build_view.py` 反映。
...
### [x] T08g — 卷004 y25(四十二年, 1年/3,726字・長文単独) [Codex]
結果: ドレイン3巡目 wave1 並列(Agent B=卷004 専任)。赧王中 四十二年(前273)pass(R2)。華陽の戦い・段干子の南陽(実=脩武)割地と蘇代の諫言・韓釐王薨/桓恵王立・黄歇(春申君)上書による秦の伐楚中止と楚秦親交。R1 で『從而伐齊/伐趙』の『從而』を「越/韓魏に従って(主導誤り)」とした2件→「そのまま」へ、『四國必悉起兵而應王』の『應王』を「王に立ち向かう」と誤訳→「王の動きに呼応」へ是正。continuity y24→y25 連鎖。これで卷004 完走。
